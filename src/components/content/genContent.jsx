...
import getContent from "../../api/content/contentAPI";
...
const ContentGen = () => {
  ...
  onSuccess: (data) => {
    setGeneratedContent(data.response); // or data.response
  },
  onError: (err) => {
    console.error(err);
  },
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
        mutation.mutate(`generate content on the topic ${values.prompt} within the category of ${values.category} in the tone ${values.tone} `)
      // Simulate content generation based on form values
    //   setGeneratedContent(`Generated content for prompt: ${mutation?.data?.response}`);
    },
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        {mutation.isPending && (
          <StatusMessage type="loading" message="Generating content, please wait..." />
        )}
        {mutation.isError && (
