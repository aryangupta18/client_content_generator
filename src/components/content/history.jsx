    ...
    <div className="bg-gray-100 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          {data.foundUser.history.length <= 0 ? 
          (<h1>No history found.</h1>):
          <ul className="divide-y divide-gray-200">
            ({data.foundUser.history.map((content, index)=>{
                return (
                    <li key={index} className="px-6 py-4 flex items-center justify-between space-x-4">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {content.content}
                </p>
                <p className="text-sm text-gray-500">{content.createdAt.toLocaleString()}</p>
              </div>
